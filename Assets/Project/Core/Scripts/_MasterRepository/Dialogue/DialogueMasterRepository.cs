...
{
    /// <summary>
    /// ダイアログのマスターデータを管理するリポジトリクラス
    /// </summary>
    public sealed class DialogueMasterRepository : IDialogueMasterRepository
    {
        // キャッシュされたマスターテーブル
        private DialogueMasterTable _table;

        // ロードしたアセットを格納する非同期操作ハンドル
        private AsyncOperationHandle<DialogueMasterTableAsset> _handle;
        ...
        public async UniTask<IDialogueMasterTable> FetchTableAsync()
        {
            // キャッシュがある場合はそれを返す
            if (_table != null)
                return _table;
            ...
            _handle = Addressables.LoadAssetAsync<DialogueMasterTableAsset>("DialogueMasterTableAsset");
            ...
            if (_handle.Status != AsyncOperationStatus.Succeeded)
            {
                ...
                throw new Exception($"Failed to load asset. Name: {nameof(DialogueMasterTableAsset)}");
            }
